...
def add_temporal_features(exog_df, dates):
    ...
    exog['month'] = dates_pd.dt.month
    exog['quarter'] = dates_pd.dt.quarter
    exog['year'] = dates_pd.dt.year
    exog['day_of_year'] = dates_pd.dt.dayofyear
    
    return exog

def create_residual_features_enhanced(residuals, exog_df, series, lag_periods=5):
    ...
    for window in [3, 5]:
        ...
        feature_dict[f'residual_std_{window}'] = pd.Series(residuals).rolling(window=window, min_periods=1).std().values
    
    # Series lags and rolling stats
    series_vals = series.values if isinstance(series, pd.Series) else series
    for lag in range(1, 4):
        feature_dict[f'series_lag_{lag}'] = np.concatenate([[np.nan] * lag, series_vals[:-lag]])
    
    for window in [3, 5, 10]:
        feature_dict[f'series_ma_{window}'] = pd.Series(series_vals).rolling(window=window, min_periods=1).mean().values
        feature_dict[f'series_std_{window}'] = pd.Series(series_vals).rolling(window=window, min_periods=1).std().values
    
    # Exogenous features with lags
    for col in exog_df.columns:
        feature_dict[col] = exog_df[col].values
...
def create_residual_features(residuals, exog_features, series, lag_periods=5):
    """Create enhanced features for XGBoost from ARIMA residuals and exogenous data"""
    feature_dict = {}
    ...
    # Original series lags and rolling stats
    series_vals = series.values if isinstance(series, pd.Series) else series
    for lag in range(1, 4):
        feature_dict[f'series_lag_{lag}'] = np.concatenate([[np.nan] * lag, series_vals[:-lag]])
...
def new_production_forecasting_pipeline(input_data, models_dict, forecast_steps, date_col='date', freq='M'):
    ...
    for indicator, model_pack in models_dict.items():
        if any('arima' in x for x in model_pack.keys()):
            arima_models[indicator] = model_pack
        elif any('prophet' in x for x in model_pack.keys()):
            prophet_models[indicator] = model_pack
        else:
            arima_models[indicator] = model_pack  # Default to ARIMA treatment


    # 3. Forecast ARIMA models first (they need exogenous variables)
    for indicator, model_pack in arima_models.items():
        try:
    ...
    for indicator, model_pack in prophet_models.items():
        try:
            prophet_model = model_pack.get('prophet_model', None)
            xgb_model = model_pack.get('xgb_model', None)
            # Create future dataframe for Prophet
            future_df = prophet_model.make_future_dataframe(periods=forecast_steps, freq=freq)
